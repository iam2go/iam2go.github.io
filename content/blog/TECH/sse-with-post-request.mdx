---
title: POST 요청을 사용한 SSE 구현기
date: "2024-11-14"
tags: [SSE]
description:  SSE를 구현해본 경험을 잊지 않기 위해 남기는 기록입니다. 근데 이제 POST 요청을 곁들인.
...
평소 프로젝트에서는 주로 Axios를 활용했지만, Axios는 POST 방식으로 SSE (Server-Sent Events)를 지원하지 않기 때문에 이번엔 **fetch API**를 사용하여 구현했다. 

동일한 이유로, 일반적으로 SSE를 구현할때 `EventSource` 객체를 많이 사용하지만 `EventSource`는 GET 요청만 지원하므로 때문에 `ReadableStream`을 사용하여 SSE를 구현했다.
